...
render_panel <- function(
  ...
  gap_AAC_to_arrow_frac = 0.12,   # AAC ↔ arrows gap (fraction of plot width)
  gap_label_to_AAC_frac = 0.08,   # left labels ↔ AAC gap (fraction of plot width)
  ...
  labels_cex <- S(0.92); header_cex <- S(1.50); row3_cex <- S(1.35)
  over_cex <- S(1.55); right_cex <- S(1.50); aac_cex <- S(1.35)
  ...
  # helpers
  f2 <- function(x, d = 2) ifelse(is.finite(x), sprintf(paste0("%.", d, "f"), x), "NA")
...
base_all <- if ("value2" %in% names(sil_df)) {
  sil_df$value2
} else if ("value" %in% names(sil_df)) {
  sil_df$value
} else {
  sil_df$sil_width
}
...
for (cc in clv) {
  ...
  build_label <- function(i){
    ...
    paste0(sil_plot$name[i], "#", sil_plot$neighborC[i], " (", paste(parts, collapse = "|"), ")")
  }
  ...
  text(0.5, 0.43, row1, adj = 0.5, cex = over_cex, font = 2, col = "red")
  text(0.05, 0.10, "adj.C (n|edge|SS|C#|nov)", adj = 0.8, cex = row3_cex, font = 2)
  ...
  text(0.88, 0.10, "Q indicator(SS|Qw|Qu|n)", adj = 0, cex = row3_cex, font = 2)
...
# EXACTLY on the left edge; text grows to the RIGHT (inside the bar)
text(x = left_edge, y = y_pos, labels = nn, adj = c(0, 0.5), cex = labels_cex)
...
by_rows <- split(seq_len(n_bar), factor(sil_plot$carac, levels = as.integer(present_cl_chr)))
  ...
  make_str <- function(cc){
    list(ss = sprintf("C%s SS=%s", cc, f2(ss_per[cc])),
         qw = if (is.finite(qw_per[cc])) sprintf("|%.2f", qw_per[cc]) else "|",
         qu = if (is.finite(qu_per[cc])) sprintf("|%.2f", qu_per[cc]) else "|",
         n  = sprintf("|%d", n_full[cc]))
  }
